...
<body>
    ...
    <form id="contact-form" method="post">
        ...
        <input type="radio" id="female" name="gender" value="female"
            <?php echo isset($gender) && $gender === 'female' ? 'checked' : ''; ?> required>
        <label for="female">Female</label><br><br>

        <label for="address">Địa chỉ:</label>
        <select id="address" name="address">
            <option value="Hà Nội" <?php echo isset($address) && $address === 'Hà Nội' ? 'selected' : ''; ?>>Hà Nội
            </option>
            <option value="TP.HCM" <?php echo isset($address) && $address === 'TP.HCM' ? 'selected' : ''; ?>>TP.HCM
            </option>
            <option value="Huế" <?php echo isset($address) && $address === 'Huế' ? 'selected' : ''; ?>>Huế</option>
            <option value="Đà Nẵng" <?php echo isset($address) && $address === 'Đà Nẵng' ? 'selected' : ''; ?>>Đà Nẵng
            </option>
        </select><br><br>

        <label for="notes">Ghi chú:</label><br>
        <textarea id="notes" name="notes" rows="4"
            cols="50"><?php echo isset($notes) ? $notes : ''; ?></textarea><br><br>
